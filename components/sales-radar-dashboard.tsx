...
import { formatMoney, initialLeads, type Lead, type LeadStatus, type ScoreResult } from "@/lib/salesradar-data";
...
export function SalesRadarDashboard() {
  ...
  async function analyzeSignal() {
    ...
    try {
      ...
      setTesterResult({
        ...
        mobileService: true,
        vehicle: null,
        reason: "Strong purchase language, same-day urgency and mobile-service intent.",
        suggestedReply: "We can help with that and come to you in Phoenix. What year, make and model is the vehicle?",
      });
    } finally {
      setAnalyzing(false);
    }
  ...
  function addTestLead() {
    ...
    const next: Lead = {
      id: Date.now(),
      score: testerResult.leadScore,
      service: testerResult.service || "Auto glass opportunity",
      city: testerResult.location || "Phoenix",
      source: "Signal tester",
      age: "now",
      text: testerText,
      urgency: testerResult.urgency >= 0.8 ? "High" : "Medium",
      valueMin: 300,
      valueMax: 700,
      status: "New",
      vehicle: testerResult.vehicle || undefined,
      tags: [testerResult.mobileService ? "Mobile requested" : "Service intent", "AI analyzed"],
      reason: testerResult.reason,
      reply: testerResult.suggestedReply,
    };
      ...
      <section className="content">
        ...
        <div className="page">
          ...
          <section className="kpiGrid">
            ...
            <article><div className="kpiTop"><span>Open pipeline</span><i className="trend">EST.</i></div><strong>{formatMoney(pipeline)}</strong><p>Midpoint value across active opportunities</p></article>
            <article><div className="kpiTop"><span>Won revenue</span><i className="trend up">TRACKED</i></div><strong>{formatMoney(won)}</strong><p>Revenue attributed to SalesRadar leads</p></article>
          ...
          <section className="radarLayout">
                ...
                {shown.length === 0 ? <div className="emptyState"><strong>No matching opportunities</strong><span>Try changing your search or lead filters.</span></div> : shown.map((lead) => (
                  <button key={lead.id} className={`leadItem ${selected.id === lead.id ? "selected" : ""}`} onClick={() => setSelectedId(lead.id)}>
                    ...
                    <div className="leadContent">
                      ...
                      <p>{lead.text}</p>
                      <div className="leadMeta"><span>⌖ {lead.city}, AZ</span><span>◉ {lead.source}</span>{lead.vehicle && <span>◇ {lead.vehicle}</span>}<span className="value">{formatMoney(lead.valueMin)}–{formatMoney(lead.valueMax)}</span></div>
                    </div>
            ...
            <aside className="detailCard">
              ...
              <div className="factsGrid"><div><span>Urgency</span><b>{selected.urgency}</b></div><div><span>Est. value</span><b>{formatMoney(selected.valueMin)}–{formatMoney(selected.valueMax)}</b></div><div><span>Vehicle</span><b>{selected.vehicle || "Needs qualification"}</b></div><div><span>Territory</span><b>In service area</b></div></div>
